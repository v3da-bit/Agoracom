...
import { BansPagination } from '@/requests/Profile';
...
function BansPage() {
    ...
    const fetchData = async (item: any, page: any) => {
        ...
        const response = await BansPagination(item, page)
        ...
        if (response.status === 200) {
            ...
            response.data.map((value: any) => {
                dynamicData.push({
                    key: value?.id,
                    user: value?.user.username ? value.user.username : value.ip_address,
                    reason: value?.violation ? <div className=' text-center grid grid-flow-row gap-1'><Link href={'/'} className='pr-2 font-semibold text-secondary-400 text-center'>Violation</Link><h1>{value.reason}</h1></div> : <h1 className=' text-center'>{value.reason}</h1>,
                    author: value?.created_by.username,
    ...
    const reFetchData = async (item: any, page: any) => {
        // setGlobalLoader(true);
        const response = await BansPagination(item, page)
        // setGlobalLoader(false);
        if (response.status === 200) {
    ...
    const columns = [
        {
            ...
            render: (record: any) => <Link href={'/'} className='text-secondary-400'>{record}</Link>
        }, {
            title: <h1 className=' text-center'>Reason For Ban</h1>,
            dataIndex: 'reason',
    ...
    return <div className='w-full p-6 bg-white'>
        <h3 className='mt-2 mb-6 text-2xl font-bold'>Bans</h3>
        {globalLoader ?

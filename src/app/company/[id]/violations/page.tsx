...
import EditPage from './[type]/edit/page';
import ViewPage from './[type]/page';
...
function ViolationPage() {
    ...
    const companyId: any = params.id
    const fetchData = async (item: any = pageSize, page: any = 1) => {
        setGlobalLoader(true);
        const response = await ViolationPagination(item, page, companyId)
        ...
        if (response.status === 200) {
            ...
            response.data.map((value: any) => {
                ...
                dynamicData.push({
                    ...
                    process: value?.status_label === 'Pending' ? <a onClick={() => {
                        setEdit(true)
                        setType('edit')
                        setId(value?.id)
                    }} className='pr-2 font-semibold text-secondary-400'>Process</a> : <a onClick={() => {
                        setEdit(true)
                        setType('view')
                        setId(value?.id)
                    }} className='pr-2 font-semibold text-secondary-400'>View</a>,
    ...
    return <div className='w-full p-6 bg-white'>
        ...
        {globalLoader ?
            ...
            : (
                edit && type == 'edit' ?
                    <EditPage fetchListData={fetchData} id={id} setEdit={setEdit} />
                    : edit && type == 'view' ?
                        <ViewPage id={id} setEdit={setEdit} />
                        :

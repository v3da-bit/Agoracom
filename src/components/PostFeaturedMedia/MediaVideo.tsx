...
import { SpeakerWaveIcon, SpeakerXMarkIcon, ArrowsPointingOutIcon } from "@heroicons/react/24/outline";
...
const MediaVideo: FC<MediaVideoProps> = ({ videoUrl, isHover, setCurrentReel, post = null }) => {
  ...
  return (
    <div className="nc-MediaVideo">
      ...
      {isPlaying && (
        <div
          className={`absolute z-20 bottom-2 end-2 h-6 rounded-full bg-black bg-opacity-70 text-white flex items-center justify-center text-sm transform transition-transform ${
            showDescUnmuted ? "ps-[6px] pe-2" : "w-6 hover:scale-125"
          }`}
          onClick={() => setCurrentReel(post)}
        >
          {isMuted ? (
            <>
